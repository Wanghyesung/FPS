...
=======
- **Passes:** "The CombatPresenter processes damage in LateUpdate, not Update, so any health modification in Update will be overwritten"
>>>>>>> Stashed changes
...
=======
- **Passes:** "The InventoryPresenter expects item IDs to be registered in ItemRegistry before any InventoryModel is created — initialization order matters"
- **Passes:** "Scene transitions must go through ScenePresenter.LoadAsync, not SceneManager directly, because ScenePresenter unsubscribes cross-scene static events before the unload"
>>>>>>> Stashed changes
...
=======
> "The damage formula in CombatPresenter uses a lookup table because the designer wanted non-linear scaling curves that couldn't be expressed as a simple formula."

**Workflow** — Procedural knowledge about HOW to do something in this project.
> "To add a new enemy type: 1) Create SO in Assets/Data/Enemies from EnemyDefinition template, 2) Add entry to EnemyRegistry SO, 3) Create prefab variant from EnemyBase prefab, 4) Register in WaveSpawnerPresenter spawn table."
>>>>>>> Stashed changes
...
=======
| **architecture** | Structural changes to Models, Views, or Presenters | Multiple MVP files edited; assembly definition changes |
>>>>>>> Stashed changes
