...
=======
> ⚠️ **프로젝트 결정 사항:** 이 프로젝트는 **VContainer(DI)를 사용하지 않습니다.** 의존성은 직접 참조(SerializeField, GetComponent) 또는 절제된 싱글톤으로 연결합니다. 프리젠터 간 느슨한 통신(이벤트)에는 **C# 이벤트(`event Action<T>`)**를 표준으로 사용합니다 — 별도 메시징 라이브러리는 쓰지 않습니다.
>>>>>>> Stashed changes
...
=======
- **Event System:** 점수 갱신, 플레이어 피격, 게임 오버, 레벨업 UI, 조커 카드 UI 등 UI와 프리젠터 간의 느슨한 결합(Observer 패턴)은 아래 **C# 이벤트(`event Action<T>`)**로 통일해서 구현할 것. 인스펙터에서 직접 바인딩해야 하는 UI 클릭 등은 `UnityEvent`를 써도 됨.
...
```
Model     — 순수 C# 클래스. 상태 + 데이터만 포함. Unity API 없음, MonoBehaviour 없음.
View      — MonoBehaviour. Model을 읽고, 비주얼을 렌더링하며, 입력을 Presenter로 전달. 로직 없음 (Passive View).
Presenter — 로직을 담당. MonoBehaviour여도 되고 순수 C#이어도 됩니다. Model을 소유/변경하고, View의 Refresh()를 호출해 갱신시킵니다.
```

Presenter는 View와 Model 사이를 중재합니다 — View는 스스로 아무것도 판단하지 않고, 입력을 Presenter에 그대로 전달한 뒤 Presenter가 지시하는 대로만 갱신됩니다(Passive View). 그래서 View ↔ Presenter는 양방향 참조이고, Model → 어디에도 참조하지 않습니다.
...
=======
// --- Presenter (MonoBehaviour, Model을 직접 소유) ---
...
=======
- Model은 절대 View나 Presenter를 참조하지 않는다
- View는 로직을 갖지 않는다 — Presenter가 호출하는 `Refresh()` 같은 순수 표시 메서드만 가진다 (Passive View)
- Presenter 간 참조는 `[SerializeField]`로 인스펙터에서 직접 연결하거나, 같은 GameObject/부모-자식 관계면 `GetComponent`/`GetComponentInParent`로 캐싱해 사용한다
- 정말 전역적으로 필요한 매니저급(예: `AudioManager`, `SaveManager`, `ObjectPoolManager`)에 한해 `public static Instance` 싱글톤을 허용한다. 단, 아래 "싱글톤 사용 가이드"를 따를 것
- 서로 다른 기능 영역(예: 전투 Presenter와 UI Presenter)을 느슨하게 연결할 때는 직접 참조 대신 **C# 이벤트**를 사용한다 (아래 "프리젠터 간 통신을 위한 C# 이벤트" 참고)
>>>>>>> Stashed changes
...
=======
- **허용:** 씬 전체에 정말 하나만 존재해야 하고, 여러 Presenter가 공통으로 참조하는 매니저급 클래스 (`AudioManager`, `SaveManager`, `ObjectPoolManager` 등)
- **금지:** `PlayerPresenter`, `ScorePresenter`처럼 특정 기능 하나를 담당하는 클래스를 습관적으로 싱글톤화하는 것. 이런 건 씬 안에서 `[SerializeField]` 직접 참조로 연결하세요
>>>>>>> Stashed changes
...
프로젝트 전반의 느슨한 결합(Observer 패턴)은 `event Action` / `event Action<T>`로 구현합니다. 별도 메시징 라이브러리(MessagePipe 등)는 쓰지 않습니다 — 씬 하나에 존재하는 Presenter 수가 많지 않아 DI 컨테이너 없이도 구독/해제를 직접 관리할 수 있기 때문입니다.
>>>>>>> Stashed changes
...
=======
public sealed class WaveSpawnerPresenter : MonoBehaviour
>>>>>>> Stashed changes
{
...
=======
View는 가벼워야 합니다 — 로직은 Presenter에, 데이터는 Model에 있어야 합니다.
>>>>>>> Stashed changes
...
=======
> ⚠️ **개발자 액션 필요:** 이 프로젝트에는 아직 New Input System 패키지가 설치돼 있지 않습니다 (`ProjectSettings/InputManager.asset`만 존재 — 레거시 입력 매니저). 아래 `InputView`/`PlayerControls` 예시는 New Input System 설치를 전제로 합니다. Unity 에디터에서 **Window → Package Manager → Unity Registry → Input System** 설치 후 프로젝트 재시작(Active Input Handling을 "Input System Package (New)" 또는 "Both"로 전환하라는 프롬프트가 뜨면 수락)이 먼저 필요합니다.

입력은 **View 계층의 관심사**입니다. InputView가 원시 입력을 읽고 Presenter를 직접 호출합니다.

```csharp
// InputView — New Input System과 게임 Presenter 사이의 얇은 어댑터
public sealed class InputView : MonoBehaviour
{
    [SerializeField] private PlayerPresenter m_refPlayerPresenter; // 인스펙터에서 직접 연결
    [SerializeField] private UIPresenter m_refUIPresenter;
>>>>>>> Stashed changes
...
=======
        m_refPlayerPresenter.SetMoveInput(vMove);
    }
    ...
    private void OnPause(InputAction.CallbackContext _ctx) => m_refUIPresenter.TogglePause();
>>>>>>> Stashed changes
}
```
...
=======
- **InputView는 View입니다** — 입력을 읽고 Presenter를 호출합니다. 게임 로직은 전혀 없습니다
- **Presenter는 입력에 대해 알지 못합니다** — `SetMoveInput(Vector2)`, `Jump()`, `Attack()` 같은 메서드를 노출합니다. 입력이 어디서 오는지(키보드, 게임패드, AI, 네트워크 리플레이) 절대 알지 못합니다
>>>>>>> Stashed changes
...
=======
- **액션 맵 전환은 InputView에 있습니다** — 메서드 호출을 통해 Presenter가 제어합니다 (예: `SwitchToUI()`, `SwitchToGameplay()`)
>>>>>>> Stashed changes
...
- View는 Presenter를 직접 참조해 입력을 전달합니다 (`[SerializeField]` 또는 `GetComponent`). Presenter는 반대로 View를 참조해 `Refresh(Model)`을 호출하고 결과를 반영시킵니다 — 즉 View ↔ Presenter는 양방향 참조입니다 (Passive View)
- Presenter는 Model과 다른 Presenter를 직접 참조하거나, 매니저급 싱글톤을 통해 접근합니다
>>>>>>> Stashed changes
...
// SpawnSystem — 적 웨이브
=======
// 좋은 예 — 책임별로 나뉜 별도의 Presenter
// PlayerPresenter — 체력, 이동
// ScorePresenter — 점수, 콤보
// SpawnPresenter — 적 웨이브
>>>>>>> Stashed changes
...
=======
- 게임 씬은 그 위에서 필요한 Presenter/View만 배치
>>>>>>> Stashed changes
